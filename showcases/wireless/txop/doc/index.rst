...
In `EDCA <https://en.wikipedia.org/wiki/IEEE_802.11e-2005#Enhanced_distributed_channel_access_(EDCA)>`__,
a TXOP is a limited time period of contention-free channel access available to the
channel-owning station. During such a period the station can send multiple frames
belonging to a particular access category.

The benefit of TXOP is that it increases throughput and decreases delay of QoS data
frames due to the lack of contention periods between transmissions. TXOP can be used
in combination with aggregation and block ack to further increase throughput. TXOP,
aggregation and block ack can be enabled independently.

By default, stations can take advantage of TXOP only for the video and voice access
categories.

More precisely, access categories have different channel access parameters,
such as `AIFS <https://en.wikipedia.org/wiki/Arbitration_inter-frame_spacing>`__
duration, contention window size, and TXOP limit. In the default EDCA OFDM parameter
...
The background and best effort categories have a TXOP limit of 0, meaning they can
send only one MSDU before having to contend for channel access again.
...
In INET, the TXOP is automatically used by default with high priority packets
when using HCF (``qosStation = true`` in the MAC). The TXOP limit for an access
...
and block ack thresholds are configured to get the following frame exchanges:
...
(alternating between 3400B and 3500B) packets to ``host2``:
...
The gap between the frames is a `SIFS <https://en.wikipedia.org/wiki/Short_Interframe_Space>`__;
