...
const Ptr<const IFunction<WpHz, Domain<simtime_t, Hz>>> DimensionalAnalogModelBase::computeReceptionPower(const IRadio *receiverRadio, const ITransmission *transmission, const IArrival *arrival) const
{
    ...
    Ptr<const IFunction<double, Domain<simtime_t, Hz>>> attenuationFunction = makeShared<AttenuationFunction>(radioMedium, transmitterAntennaGain, receiverAntennaGain, transmissionStartPosition, receptionStartPosition, frequencyQuantization);
    if (attenuateWithCarrierFrequency)
        attenuationFunction = makeShared<ConstantFunction<double, Domain<simtime_t, Hz>>>(attenuationFunction->getValue(Point<simtime_t, Hz>(arrival->getStartTime(), dimensionalTransmission->getCarrierFrequency())));
