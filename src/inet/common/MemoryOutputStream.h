...
class INET_API MemoryOutputStream {
  ...
  public:
    ...
    /**
     * Writes a string with the maximum length of n bytes to the end of the stream
     * without '\0'.
     */
    void writeStringOfMaxNBytes(const char* str, int n){
        ...
        if(n < (uint8_t)(sizeof(str) / sizeof(str[0]))) throw cRuntimeError("String is longer than maximum length.");
